...
CaveBot.Extensions.SupplyCheck.setup = function()
  CaveBot.registerAction(
    ...
    function(value)
      ...
      if pos then
        if missedChecks >= 10 then
          missedChecks = 0
          supplyRetries = 0
          print("CaveBot[SupplyCheck]: Missed 5 supply checks, proceeding with waypoints")
          return true
        end
        if getDistanceBetween(player:getPosition(), pos) > 15 then
          missedChecks = missedChecks + 1
          print("CaveBot[SupplyCheck]: Missed supply check! " .. 10 - missedChecks .. " tries left before skipping.")
