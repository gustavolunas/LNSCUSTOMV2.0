  ...
  CaveBot.registerAction(
    ...
    function(value)
      ...
      elseif storage.caveBot.backOffline then
        ...
        missedChecks = 0
        return false
      elseif supplyRetries > (charStorage.extras.huntRoutes or 50) then
        print("CaveBot[SupplyCheck]: Round limit reached, going back on refill. Last round took: " .. round)
        setCaveBotData()
        return false
      elseif cavebotCheckImbueByLook and cavebotCheckImbueByLook() == "REFRESH" then
        print("CaveBot[SupplyCheck]: Auto Imbuement precisa de refresh. Going on refill. Last round took: " .. round)
        setCaveBotData()
        return false
      ...
      elseif (supplyInfo.capacity.enabled and freecap() < tonumber(supplyInfo.capacity.value)) then
        ...
        print("CaveBot[SupplyCheck]: Enough supplies. Hunting. Round (" .. supplyRetries .. "/" .. (charStorage.extras.huntRoutes or 50) .. "). Last round took: " .. round)
        setCaveBotData(true)
        return CaveBot.gotoLabel(label)
      end
    end
