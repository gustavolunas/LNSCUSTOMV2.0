...
charStorage.extras = charStorage.extras or {}

caveCharStorage = loadCaveCharStorage()
caveCharStorage.selectedProfile = caveCharStorage.selectedProfile or ""

local function saveCaveChar()
  saveCaveCharStorage(caveCharStorage)
end

storage._configs = storage._configs or {}
storage._configs.cavebot_configs = storage._configs.cavebot_configs or {}

if caveCharStorage.selectedProfile ~= "" then
  storage._configs.cavebot_configs.selected = caveCharStorage.selectedProfile
end

local function getExtraGotoMaxDistance()
  charStorage.extras = charStorage.extras or {}
  return tonumber(charStorage.extras.gotoMaxDistance) or 30
end
...
config = Config.setup("cavebot_configs", configWidget, "cfg", function(name, enabled, data)
  ...
  if enabled == true and name and name ~= "" then
    caveCharStorage.selectedProfile = name
    saveCaveChar()
...
CaveBot.getCurrentProfile = function()
  return caveCharStorage.selectedProfile or ""
end
  ...
  for i, child in ipairs(actions) do
    if i > index then
      ...
      if string.starts(text, "goto:") then
        ...
        if posz() == pos.z then
          local maxDist = getExtraGotoMaxDistance()
  ...
  for i = 0, 100 do
    ...
    if child then
      ...
      if string.starts(text, "goto:") then
        ...
        if posz() == pos.z then
          local maxDist = getExtraGotoMaxDistance()
          if distanceFromPlayer(pos) <= maxDist / 2 then
...
CaveBot.save = function()
  ...
  if storage._configs and storage._configs.cavebot_configs and storage._configs.cavebot_configs.selected then
    caveCharStorage.selectedProfile = storage._configs.cavebot_configs.selected
    saveCaveChar()
  end
