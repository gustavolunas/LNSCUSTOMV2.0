...
local function pathfinder()
  if not charStorage.extras.pathfinding then return end
...
CaveBot.registerAction("goto", "green", function(value, retries, prev)
  ...
  local maxDist = charStorage.extras.gotoMaxDistance or 40
  ...
  if not path then
    if breakFurniture(pos, charStorage.extras.machete) then
      CaveBot.delay(1000)
      retries = 0
      return "retry"
    end
