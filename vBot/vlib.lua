...
CachedFriends = {}
CachedEnemies = {}
local function getPlayerListStorage()
  ...
  return charStorage.playerList
end

function isFriend(c)
    local playerList = getPlayerListStorage()
    ...
    if table.find(playerList.friendList, name) then
        CachedFriends[c] = true
        return true
    elseif vBot.BotServerMembers[name] ~= nil then
        CachedFriends[c] = true
        return true
    elseif playerList.groupMembers then
        local p = c
...
function isEnemy(c)
    ...
    if p:isPlayer() and table.find(playerList.enemyList, name) or
       (playerList.marks and not isFriend(name)) or
       p:getEmblem() == 2 then
        return true
    else
        return false
