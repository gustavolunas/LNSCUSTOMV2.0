...
local settings = charStorage[panelName]

local function saveExtrasSettings()
...
addCheckBox("pathfinding", "CaveBot Pathfinding", false, leftPanel, "Cavebot will automatically search for first reachable waypoint after missing 10 goto's.")
...
addCheckBox("lootLast", "Start loot from last corpse", false, leftPanel, "Looting sequence will be reverted and bot will start looting newest bodies.")
...
if true then
  ...
  onAddThing(function(tile, thing)
    ...
    if thing:getId() == 2129 then 
      timer = 20000
	  elseif thing:getId() == 16518 then
      timer = 20000
    elseif thing:getId() == 2130 then
      timer = 45000
    else
      return
    end
      ...
      schedule(2000, function() 
          if g_game.getClientVersion() > 1000 then
            if not blessed and player:getBlessings() == 0 then
                modules.game_textmessage.displayBroadcastMessage("!! Blessings not bought !!")
            end
          end
      end)
    else
      modules.game_textmessage.displayGameMessage("Bless Actived, are you safe!!!")
    end
  end
end
