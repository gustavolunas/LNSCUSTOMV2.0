...
if type(charStorage[panelName]) ~= "table" then
    charStorage[panelName] = {
        enabled = false;
        height = 245,
        purse = true;
        list = {
...
local openContainer = function(id)
    local t = {getRight(), getLeft(), getAmmo()}
    for i=1,#t do
        local slotItem = t[i]
...
if rootWidget then
    ...
    contListWindow:setHeight(config.height or 245)
